...
class FrechaServicesChatbot:
    ...
    def get_response(self, user_input: str) -> str:
        ...
        if any(word in user_input_lower for word in ['hello', 'hi', 'hey', 'mambo', 'habari', 'hujambo']):
            ...
            return self.handle_sme_inquiry(user_input_lower)
...
@app.route('/health')
def health():
    return jsonify({'status': 'healthy', 'service': 'Frecha iotech Chatbot'})

if __name__ == '__main__':
    port = int(os.environ.get('PORT', 5000))
    print("🚀 Starting Frecha iotech Chatbot...")
    print("📍 Server will run at: http://0.0.0.0:" + str(port))
    print("🎯 Default language: Swahili")
    print("💡 Try: mambo, bundle, vodacom, SME")
